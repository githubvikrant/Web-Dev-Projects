...
export default function Home() {
    ...
    async function sendRequest(e) {
        ...
        try {
            ...
            setTimeout(() => {
                var _a;
                (_a = responseSectionRef.current) === null || _a === void 0 ? void 0 : _a.scrollIntoView({ behavior: "smooth" });
            }, 100);
        }
        catch (err) {
    ...
    return (<div style={{ maxWidth: 800, margin: "2rem auto", fontFamily: "sans-serif" }}>
      ...
      <form onSubmit={sendRequest} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <input type="text" placeholder="Request URL" value={url} onChange={e => setUrl(e.target.value)} style={{ flex: 1 }} required/>
        </div>
        <textarea placeholder="Headers (one per line: Key: Value)" value={headers} onChange={e => setHeaders(e.target.value)} rows={2}/>
        {["POST", "PUT"].includes(method) && (<textarea placeholder="Request body (raw)" value={body} onChange={e => setBody(e.target.value)} rows={4}/>)}
      ...
      <div style={{ marginTop: 32, maxHeight: 300, overflowY: 'auto', border: '1px solid #ccc', borderRadius: 4, background: '#fff', padding: 16, color: '#111' }}>
        ...
        {(!history || history.length === 0) ? (<div style={{ color: "#888" }}>No history yet.</div>) : (<ul style={{ listStyle: "none", padding: 0 }}>
            {history.map((h, i) => (<li key={h.id} style={{ marginBottom: 12, padding: 8, border: "1px solid #eee", borderRadius: 4 }}>
        ...
        {/* Lazy Load Button */}
        {hasMore && !historyError && (<div style={{ marginTop: 8 }}>
